...
class Goodsup extends MobileBase
{
    ...
    public function getStoremes(){
        ...
        $storeid = $storemes['storeid'] ;
        $goodpinmes = db('goods')->where(array('supplier_uid'=>$storeid,'is_on_sale'=>1))->select(); //获取商家对应的店铺商品信息
        $storemes['xiangaddress'] = $storemes['provinces'].$storemes['citys'].$storemes['districts'].$storemes['street'] ;
        foreach ($goodpinmes as $kps => $vaps) {
            $goodpinmes[$kps]['original_img'] = $vaps['store_ico'] ;
        }
        $storemes['goodarr'] = $goodpinmes ;

        $storeid = $storemes['storeid'] ;
        $goodslunImages = M("sz_yi_store_images")->where('storeid ='.$storeid)->select();
        $storemes['lunimgarr'] = $goodslunImages ;

        if($storemes['type'] == 1){
            //驾校信息
            $trainermess = db('sz_yi_trainer')->where('userid',$storemes['useid'])->order('orders')->select(); //获取当前驾校教练信息
    ...
    public function gettrainermess(){
        ...
        $storeid = I('post.storeid') ; //获取商家总id
        $goodslunImages = M("sz_yi_store_images")->where('storeid',$storeid)->select();
        $trainermess['lunimgarr'] = $goodslunImages ;
    ...
    {   
        ...
        if($goods['is_discount'] == 1){
            //是折扣商品
            // $goods['shop_price'] = $goods['hui_price'] ; 
        }
    ...
    public function getGuigemes(){
        ...
        $gid = I("post.gid") ;
        $goodsmess = db('goods') -> where('goods_id',$gid) -> field('goods_id,is_discount') -> find();
        if($goodsmess['is_discount'] == 1){
            //是折扣商品
            // $specarr['price'] = $specarr['jiaprice'] ;
        }
    ...
    //获取当前驾校里面的套餐信息
    public function jiaxiaoTaocan(){
        $storeid = I('post.storeid') ; //获取商家总id
        $data['srid'] = $storeid ; //获取商家总id
        $data['is_show'] = 1 ;
        $drivermealarr = db('sz_yi_drivermeal')->where($data)->order('orders desc')->select(); //获取所有套餐信息
        $this->ajaxReturn($drivermealarr);
    }
