...
class Centerup extends MobileBase
{
    ...
    public function drivercarjoin(){
        ...
        $datajoin['tid'] = I('post.tid'); //获取套餐id

        $type = I('post.type') ; //报考类型
        if(empty($type)){
            $this->ajaxReturn(['status' => -1, 'msg' => '报考类型不能为空!']);
        }
    ...
    public function updateBaomodel(){
        ...
        // $datas['baogetfen'] = $carmess['zengjifen'] ; //赠送积分
        // $datas['baogetyu'] = $carmess['zengyue'] ; //赠送余额

        $drivermealmess = db('sz_yi_drivermeal')->where('id',$tid)->find(); //查询对应的套餐信息
        $mealprice = $drivermealmess['mealprice'] ; //套餐价格
        if($bmid == 1){
            //A模式
            $modelmoney = $mealprice ;
            $zengjifen = $mealprice ;
            $zengyue = 0 ;
        }elseif ($bmid == 2) {
            //B模式
            $modelmoney = $mealprice * 2 ;
            $zengjifen = $mealprice ;
            $zengyue = $mealprice * 1.6 ;
        }

        $datas['paymoney'] = $modelmoney ; //报名支付费用
        $datas['order_amount'] = $modelmoney ; //报名支付费用
        $datas['baogetfen'] = $zengjifen; //赠送积分
        $datas['baogetyu'] = $zengyue ; //赠送余额
        $res = db('sz_yi_carjoin')->where('id',$bid)->update($datas);
        if($res){
            $carjoinmess = db('sz_yi_carjoin')->where('id',$bid)->find();
            $shangdianmess = db('sz_yi_store_data')->where('storeid',$carjoinmess['jid'])->find();
            $is_yuepay = $shangdianmess['is_yuepay'] ; //是否支持余额支付 
            $this->ajaxReturn(['status' => 1, 'msg' => '确定成功!','modelmoney'=>$modelmoney,'is_yuepay'=>$is_yuepay]);
        }else{
    ...
    //取消驾考信息
    public function cancelcarjoin(){
        $cid = I('post.cid'); //获取记录id
        // $res = db('sz_yi_carjoin')->where('id',$cid)->update(['status' => -1]);
        // if($res){
        //     $this->ajaxReturn(['status' => 1, 'msg' => '取消成功!']);
        // }else{
        //     $this->ajaxReturn(['status' => -1, 'msg' => '取消失败!']);
        // }
        $res = db('sz_yi_carjoin')->where('id',$cid)->delete();
        if($res){
    ...
    public function addHuoaddress(){
        ...
        if($op == 'edit'){
            ...
            // }
            if($address['is_default'] == 1 && $is_default == 1){
                //编辑的时候本来就是默认地址
                M('user_address')->where(array('user_id'=>$userid))->save(array('is_default'=>0));
                $data['is_default'] = 1 ;
            }
    ...
    public function baoCarjoinmess(){
        ...
        $carjoinmess = db('sz_yi_carjoin')->where('id',$pid)->find();
        if(!$carjoinmess){
            $this->ajaxReturn(['status' => -1, 'msg' => '还没报名!']);
        }
        $carjoinmess['baotime'] = date('Y-m-d H:i:s',$carjoinmess['baotime']);
        // $carjoinmess['chosejiatime'] = date('Y-m-d H:i:s',$carjoinmess['chosejiatime']);
        $jiaxiaomess = db('sz_yi_store_data')->where('storeid',$carjoinmess['jid'])->field('storename,is_yuepay')->find(); //获取当前驾校信息
        ...
        $carjoinmess['is_yuepay'] = $jiaxiaomess['is_yuepay'] ; //是否支付余额支付
    ...
    public function pingTrainermess(){
        ...
        $trainermess = db('sz_yi_trainer')->where('id',$jid)->find(); //获取教练信息
        $datac['uid'] = $trainermess['userid'] ;
    ...
    public function getFenyuedetail(){
        $userid = $this->getUsemes();
        $jifendetail = db('sz_yi_redpointsdetail')->where('userid',$userid)->order('addtime desc')->select(); //获取当前用户的积分明细
        foreach ($jifendetail as $kh => $vah) {
            if($vah['type'] == 1){
                ...
                $jifendetail[$kh]['typename'] = '用户注册赠送积分' ;
            }elseif ($vah['type'] == 6) {
                $jifendetail[$kh]['typename'] = '商家入驻赠送积分' ;
            }elseif ($vah['type'] == 7) {
                $jifendetail[$kh]['typename'] = '驾校入驻赠送积分' ;
            }
        }
        $arr['jifendetail'] = $jifendetail ;
        $yuedetail = db('sz_yi_yuezhimes')->where('userid',$userid)->order('paytime desc')->select(); //获取当前用户的余额明细
    ...
    public function getShiyongjuan(){
        $userid = $this->getUsemes();
        $usecodemess = db('sz_yi_usecode')->where('userid',$userid)->order('starttime desc ')->select(); //获取当前用户的使用劵的信息
    ...
    //查询当前用户对应的记录信息
    public function lookDuiapplicelog(){
        $jid = I('post.jid') ;
        $applicemess = db('sz_yi_af_supplier')->where('id',$jid)->find(); //获取对应用户的申请记录信息
        $categorymess = db('sz_yi_supplier_category')->where('id',$applicemess['cate_id'])->field('id,name')->find();
        $applicemess['catename'] = $categorymess['name'] ;
        $this->ajaxReturn($applicemess);
    }

    //提交所要更新的记录信息
    public function subQueapplicelog(){
        $jid = I('post.jid') ;

        $updatecon['jid'] = $jid ;
        $updatecon['companydai'] = I('post.companydai') ;
        $updatecon['companycode'] = I('post.companycode') ;
        $updatecon['certificateimg'] = I('post.certificateimg') ;
        $updatecon['certificateimg1'] = I('post.certificateimg1') ;
        $updatecon['licensenum'] = I('post.licensenum') ;
        $updatecon['accountimg'] = I('post.accountimg') ; //营业执照正本
        $updatecon['licenseimg'] = I('post.licenseimg') ; //营业执照副本
        $updatecon['sex'] = I('post.sex') ; //性别
        $updatecon['yingstarttime'] = I('post.yingstarttime') ; //营业开始时间
        $updatecon['yingendtime'] = I('post.yingendtime') ; //营业结束时间
        $ress = db('sz_yi_af_resupplier')->add($updatecon);
        if($ress){
            $this->ajaxReturn(['status'=>1,'msg'=>'提交成功!']); 
        }else{
            $this->ajaxReturn(['status'=>-1,'msg'=>'提交失败!']);
    ...
    //驾考列表信息
    public function baoJiakaomess(){
        $userid = $this->getUsemes();
        $carjoinmess = db('sz_yi_carjoin')->where('userid',$userid)->order('baotime desc ')->select();

        foreach ($carjoinmess as $ks => $vas) {
            $carjoinmess[$ks]['baotime'] = date('Y-m-d H:i:s',$vas['baotime']);
            $jiaxiaomess = db('sz_yi_store_data')->where('storeid',$vas['jid'])->field('storename')->find(); //获取当前驾校信息
            $carjoinmess[$ks]['storename'] = $jiaxiaomess['storename'] ; //驾校名字
        }
    ...
    //报名支付轮播图
    public function baoZhibanner(){
        $advment = db('ad')->where(array('pid'=>51325,'enabled'=>1))->field('ad_id,ad_name,ad_code')->select(); //查询轮播图的信息
        $this->ajaxReturn($advment);
    }
